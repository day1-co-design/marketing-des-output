...
- `comparison-dashboard.html`은 KR/JP/GL 언어별로 `코스 유형 × 현지화 유형 × 코스 포맷` 케이스를 비교해 `완전 동일`, `작업주체 차이`, `업무 차이`, `국가별 업무 확인 추가 필요`로 통합 가능성을 보여줍니다.
...
가이드 매칭용 케이스 스크린샷은 아래 명령으로 다시 생성할 수 있습니다.

```bash
node scripts/generate-kr-screenshots.mjs
```

기본값은 KR이며 생성 결과는 `outputs/{site}-screenshots/`에 저장됩니다.
...
특정 사이트를 지정하려면 `--site` 또는 `--sites` 옵션을 사용합니다.

```bash
node scripts/generate-kr-screenshots.mjs --sites=JP,GL --live-sync
```

더빙도 코스 포맷 8개를 모두 캡처해야 하면 `--all-dubbing` 옵션을 함께 사용합니다.

```bash
node scripts/generate-kr-screenshots.mjs --sites=JP,GL --live-sync --all-dubbing
```

기존 폴더에서 더빙 케이스만 다시 캡처하려면 `--only-dubbing` 옵션을 함께 사용합니다.

```bash
node scripts/generate-kr-screenshots.mjs --site=JP --live-sync --all-dubbing --only-dubbing
```
